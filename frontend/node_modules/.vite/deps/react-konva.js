...
var Transform = class _Transform {
  ...
  rotate(rad) {
    ...
    const m11 = this.m[0] * c2 + this.m[2] * s;
    const m12 = this.m[1] * c2 + this.m[3] * s;
    const m21 = this.m[0] * -s + this.m[2] * c2;
    const m22 = this.m[1] * -s + this.m[3] * c2;
    this.m[0] = m11;
    this.m[1] = m12;
    this.m[2] = m21;
    this.m[3] = m22;
  ...
  decompose() {
    ...
    if (a2 != 0 || b2 != 0) {
      ...
      result.rotation = b2 > 0 ? Math.acos(a2 / r) : -Math.acos(a2 / r);
      result.scaleX = r;
      result.scaleY = delta / r;
      result.skewX = (a2 * c2 + b2 * d) / delta;
      result.skewY = 0;
    } else if (c2 != 0 || d != 0) {
      const s = Math.sqrt(c2 * c2 + d * d);
      result.rotation = Math.PI / 2 - (d > 0 ? Math.acos(-c2 / s) : -Math.acos(c2 / s));
      result.scaleX = delta / s;
      result.scaleY = s;
      result.skewX = 0;
      result.skewY = (a2 * c2 + b2 * d) / delta;
    } else {
    }
...
var Context = class {
  ...
  setTransform(a2, b2, c2, d, e, f2) {
    this._context.setTransform(a2, b2, c2, d, e, f2);
  }
  stroke(path2d) {
    if (path2d) {
  ...
  transform(a2, b2, c2, d, e, f2) {
    this._context.transform(a2, b2, c2, d, e, f2);
  }
  translate(x2, y) {
...
function isCanvasFarblingActive() {
  const c2 = Util.createCanvasElement();
  c2.width = 1;
  c2.height = 1;
...
var Node = class _Node {
  ...
  _getTransform() {
    ...
    m2.reset();
    const x2 = this.x(), y = this.y(), rotation = Konva.getAngle(this.rotation()), scaleX = (_a = this.attrs.scaleX) !== null && _a !== void 0 ? _a : 1, scaleY = (_b = this.attrs.scaleY) !== null && _b !== void 0 ? _b : 1, skewX = this.attrs.skewX || 0, skewY = this.attrs.skewY || 0, offsetX = this.attrs.offsetX || 0, offsetY = this.attrs.offsetY || 0;
    if (x2 !== 0 || y !== 0) {
      m2.translate(x2, y);
    }
    if (rotation !== 0) {
...
var Easings = {
  ...
  BounceEaseIn(t, b2, c2, d) {
    return c2 - Easings.BounceEaseOut(d - t, 0, c2, d) + b2;
  },
  BounceEaseInOut(t, b2, c2, d) {
    if (t < d / 2) {
      return Easings.BounceEaseIn(t * 2, 0, c2, d) * 0.5 + b2;
    } else {
      return Easings.BounceEaseOut(t * 2 - d, 0, c2, d) * 0.5 + c2 * 0.5 + b2;
    }
  },
  EaseIn(t, b2, c2, d) {
    return c2 * (t /= d) * t + b2;
...
var getCubicArcLength = (xs, ys, t) => {
  ...
  for (let i2 = 0; i2 < n; i2++) {
    correctedT = z * tValues[n][i2] + z;
    sum += cValues[n][i2] * BFunc(xs, ys, correctedT);
  }
  return z * sum;
};
...
var Path = class _Path extends Shape {
  ...
  static getPointAtLengthOfDataArray(length, dataArray) {
    ...
    switch (cp.command) {
      ...
      case "C":
        return _Path.getPointOnCubicBezier(t2length(length, _Path.getPathLength(dataArray), (i3) => {
      ...
      case "Q":
        return _Path.getPointOnQuadraticBezier(t2length(length, _Path.getPathLength(dataArray), (i3) => {
  ...
  _sceneFunc(context) {
    ...
    for (n = 0; n < textArrLen; n++) {
      ...
      if (shouldLineThrough) {
        ...
        const x2 = align === JUSTIFY ? 0 : lineTranslateX;
...
var Emboss = function(imageData) {
  ...
  const lum = new Float32Array(w * h2);
  ...
  for (let x2 = 0; x2 < w; x2++) {
    let oTop = x2 * 4, oBot = ((h2 - 1) * w + x2) * 4;
    data[oTop] = src[oTop];
...
var Solarize = function(imageData) {
  ...
  for (let i2 = 0; i2 < d.length; i2 += 4) {
    const r = d[i2], g = d[i2 + 1], b2 = d[i2 + 2];
...
var Stage2 = import_react.default.forwardRef((props, ref) => {
...
export {
  ...
  Path2 as Path,
  Rect2 as Rect,
  RegularPolygon2 as RegularPolygon,
  Ring2 as Ring,
  Shape2 as Shape,
  Sprite2 as Sprite,
  Stage2 as Stage,
  ...
  Tag2 as Tag,
  Text2 as Text,
  TextPath2 as TextPath,
  Transformer2 as Transformer,
  Wedge2 as Wedge,
